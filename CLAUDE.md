...
- `--count` must satisfy `1 <= count <= 100` in both the CLI and `namegen.Generate`.
- `--raw-prefix` and custom `--stamp` layouts are trusted-input-only: preserved line breaks, control bytes, and path separators can make generated stdout unsafe for command substitution, terminals, and paths.
